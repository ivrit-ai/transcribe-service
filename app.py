...
# xhost mode: a specialization of cloud/RunPod mode for single-container agent
# hosting. No local GPU, durable state in Postgres, operator-owned OAuth.
in_xhost_mode = os.environ.get("TS_XHOST_MODE", "0") == "1"
if in_xhost_mode and in_local_mode:
    raise RuntimeError("xhost mode is incompatible with --local (no local GPU)")
if in_xhost_mode:
    _missing = [
        k for k in (
            "DATABASE_URL", "RUNPOD_API_KEY", "RUNPOD_ENDPOINT_ID",
...
_max_minutes_env = os.environ.get("TS_MAX_MINUTES_PER_WEEK")
MAX_MINUTES_PER_WEEK = (
    float('inf') if in_local_mode
    else float(_max_minutes_env) if _max_minutes_env
    else args.max_minutes_per_week
)  # Maximum credit grant per week
...
# Under xhost the operator configures only BASE_URL; derive the OAuth redirect
# from it so gdrive_auth (which reads GOOGLE_REDIRECT_URI at import) picks it up.
if in_xhost_mode and not os.environ.get("GOOGLE_REDIRECT_URI"):
    os.environ["GOOGLE_REDIRECT_URI"] = os.environ["BASE_URL"].rstrip("/") + "/login/authorized"
...
if in_xhost_mode:
    ...
    db = Database(backend="postgres", dsn=DB_URL)
else:
    _state_dir = args.data_dir if (in_local_mode and args.data_dir) else "local_data"
    _state_path = os.path.join(_state_dir, "state.db")
    DB_URL = "sqlite:///" + os.path.abspath(_state_path)
    db = Database(backend="sqlite", path=_state_path)
...
async def lifespan(app: FastAPI):
    ...
    # Apply schema migrations (sync, via Alembic), then connect the async pool.
...
# GPU selection for autogenerated (paid-tier) endpoints, in RunPod REST v2 terms:
# a pool is priced as one unit and workers land on whichever member card has
# capacity. Chosen for availability over speed — waiting for a free worker costs
...
# Statistics counters live in durable storage: db.incr_stat(key, amount) to bump,
# db.get_stats() to read the whole set. Keys are flat strings (see increment
...
async def get_user_quota(user_email):
    """Load the user's rate-limiting bucket from durable storage.

    A missing row means an untouched, fully-replenished bucket. Read paths don't
    persist: replenishment is a pure function of (last_update, seconds_remaining,
    now), so recomputing from the stored pair on the next load yields the same
    value. Only consume() changes the balance and must be saved back.
    """
    bucket = LeakyBucket(MAX_MINUTES_PER_WEEK)
    row = await db.get_quota(user_email)
    if row:
        bucket.max_seconds = row["max_seconds"]
        bucket.seconds_remaining = row["seconds_remaining"]
        bucket.last_update = row["last_update"]
    return bucket
...
async def get_audio_file(results_id: str, request: Request):
    ...
    if file_content is None:
        await db.incr_stat("gdrive_error_audio_download", 1)
...
async def rename_file(request: Request):
    ...
    try:
        ...
        await db.incr_stat("gdrive_error_rename", 1)
    ...
    try:
        ...
        if opus_file_id:
            ...
            if not delete_success:
                await db.incr_stat("gdrive_error_delete", 1)
...
async def get_max_serverless_concurrency(api_key: str) -> Optional[int]:
    ...
    # REST v2 exposes no account concurrency limit (as of 2026-08-18), so this stays on GraphQL.
    if in_local_mode:
...
async def list_runpod_endpoints(api_key: str) -> Optional[list]:
    """List all serverless endpoints using REST API v2"""
    if in_local_mode:
        raise RuntimeError("RunPod endpoint functions are not available in local mode")
    try:
        timeout = aiohttp.ClientTimeout(total=10.0)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(
                f"{RUNPOD_V2_URL}/serverless",
                headers={"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
            ) as response:
                if response.status >= 400:
                    error_body = await response.text()
                    logger.error(f"Error listing endpoints: HTTP {response.status}: {error_body[:2000]}")
                    return None
...
async def get_runpod_template(api_key: str, template_id: str) -> Optional[dict]:
    """Fetch a serverless template using REST API v2"""
    if in_local_mode:
        raise RuntimeError("RunPod endpoint functions are not available in local mode")
    try:
        timeout = aiohttp.ClientTimeout(total=10.0)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(
                f"{RUNPOD_V2_URL}/templates/{template_id}",
                headers={"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
            ) as response:
                if response.status >= 400:
                    error_body = await response.text()
                    logger.error(f"Error fetching template {template_id}: HTTP {response.status}: {error_body[:2000]}")
                    return None
...
async def get_current_worker_usage(api_key: str) -> Optional[int]:
    """Get current worker usage by summing max workers across all endpoints"""
    endpoints = await list_runpod_endpoints(api_key)
    if endpoints is None:
        return None

    total_workers = sum(endpoint["workers"]["max"] for endpoint in endpoints)
    log_message(f"Current total worker usage across {len(endpoints)} endpoints: {total_workers}")

    for endpoint in endpoints:
        log_message(f"Endpoint {endpoint['name']}: workersMax={endpoint['workers']['max']}")

    return total_workers

async def check_runpod_balance(api_key: str) -> Optional[dict]:
    """Check RunPod balance using GraphQL API"""
    # REST v2 exposes no account balance (as of 2026-08-18), so this stays on GraphQL.
    if in_local_mode:
        raise RuntimeError("RunPod endpoint functions are not available in local mode")
    GRAPHQL_URL = "https://api.runpod.io/graphql"
    GRAPHQL_HEADERS = {
...
async def find_runpod_endpoint(api_key: str) -> Optional[dict]:
    """Find the autogenerated endpoint using REST API v2"""
    endpoints = await list_runpod_endpoints(api_key)
    if endpoints is None:
        return None
    ...
    for endpoint in endpoints:
        ...
        if re.match(pattern_regex, endpoint["name"]):
            log_message(f"Found endpoint: {endpoint['id']} ({endpoint['name']}) with image {endpoint['image']}")
            return {
                "id": endpoint["id"],
                "name": endpoint["name"],
                "image": endpoint["image"],
                "pools": endpoint["gpu"]["pools"],
...
async def delete_runpod_endpoint(api_key: str, endpoint_id: str) -> bool:
    """Delete an endpoint using REST API v2"""
    if in_local_mode:
        raise RuntimeError("RunPod endpoint functions are not available in local mode")
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.delete(
                f"{RUNPOD_V2_URL}/serverless/{endpoint_id}",
                headers={"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
            ) as response:
                if response.status >= 400:
                    error_body = await response.text()
                    logger.error(f"Error deleting endpoint {endpoint_id}: HTTP {response.status}: {error_body[:2000]}")
                    return False
...
async def create_runpod_endpoint(api_key: str, template_id: str) -> Optional[dict]:
    """Create a new autogenerated endpoint using REST API v2 with dynamic worker limits"""
    if in_local_mode:
        raise RuntimeError("RunPod endpoint functions are not available in local mode")
    ...
    endpoint_data = {
        ...
        "gpu": {"pools": RUNPOD_GPU_POOLS, "count": 1},
        "flashboot": "FLASHBOOT",
        "scaling": {"type": "QUEUE_DELAY", "queueDelay": 4},
        "workers": {"min": 0, "max": workers_max, "idleTimeout": 5},
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                if response.status >= 400:
                    error_body = await response.text()
                    logger.error(f"Error creating autogenerated endpoint: HTTP {response.status}: {error_body[:2000]}")
                    return None
...
async def get_balance(request: Request):
    """Get RunPod balance for the user's stored key"""
    ...
    runpod_token = get_session_runpod_token(request)
    if not runpod_token:
...
async def get_stats(request: Request):
    ...
    drive_errors = {k: s.get(f"gdrive_error_{k}", 0) for k in GDRIVE_ERROR_KINDS}
    ...
    transcoding_stats = {
        "jobs": s.get("transcoding_jobs", 0),
        "total_gb": s.get("transcoding_total_gb", 0.0),
        "total_duration_seconds": s.get("transcoding_total_duration_seconds", 0.0),
        "total_duration_formatted": format_duration(s.get("transcoding_total_duration_seconds", 0.0) / 60.0 if s.get("transcoding_total_duration_seconds", 0.0) else 0),
    ...
    stats_data = {
        ...
        "transcribed_since_launch": {
            "short": {
                "jobs_count": s.get(f"jobs_transcribed_{SHORT}", 0),
                "total_minutes": s.get(f"minutes_transcribed_{SHORT}", 0.0),
                "total_minutes_formatted": format_duration(s.get(f"minutes_transcribed_{SHORT}", 0.0))
            },
            "long": {
                "jobs_count": s.get(f"jobs_transcribed_{LONG}", 0),
                "total_minutes": s.get(f"minutes_transcribed_{LONG}", 0.0),
                "total_minutes_formatted": format_duration(s.get(f"minutes_transcribed_{LONG}", 0.0))
            },
            "private": {
                "jobs_count": s.get(f"jobs_transcribed_{PRIVATE}", 0),
                "total_minutes": s.get(f"minutes_transcribed_{PRIVATE}", 0.0),
                "total_minutes_formatted": format_duration(s.get(f"minutes_transcribed_{PRIVATE}", 0.0))
            },
            "total": {
                "jobs_count": s.get("total_jobs_started", 0),
                "total_minutes": s.get("total_minutes_processed", 0.0),
                "total_minutes_formatted": format_duration(s.get("total_minutes_processed", 0.0))
            }
        },
        "system_info": {
            "max_parallel_jobs": {
...
async def check_runpod_endpoint(runpod_token: str) -> dict:
    ...
    try:
        ...
        # REST v2 resolves the template into the endpoint's container settings and does not
        # echo the template ID back, so up-to-dateness is judged on the settings themselves:
        # the template's image, plus the GPU pools the code wants. This also means GPU pool
        # changes propagate to existing endpoints, which a template ID comparison never did.
        template = await get_runpod_template(runpod_token, required_template_id)
        if template is None:
            return {"success": False, "error": "Failed to read the worker template"}
        required_image = template["image"]
        ...
        if endpoint_info:
            up_to_date = (
                endpoint_info["image"] == required_image
                and set(endpoint_info["pools"]) == set(RUNPOD_GPU_POOLS)
                # != "OFF" rather than == "FLASHBOOT": if RunPod upgrades an endpoint to
                # PRIORITY_FLASHBOOT, an exact match would delete/recreate it on every upload.
                and endpoint_info["flashboot"] != "OFF"
            )
            ...
            if up_to_date:
                ...
                # Endpoint config doesn't match - delete and recreate
                log_message(
                    f"Endpoint config mismatch: image={endpoint_info['image']} vs {required_image}, "
                    f"pools={endpoint_info['pools']} vs {RUNPOD_GPU_POOLS}, flashboot={endpoint_info['flashboot']}"
                )
                ...
                # Create new endpoint with correct configuration
...
@app.post("/runpod_key", dependencies=[Depends(require_google_login)])
async def save_runpod_key(request: Request):
    """Verify the user's RunPod key, store it encrypted on their Drive, and use it for this session."""
    if in_local_mode:
    ...
    runpod_token = (body.get("runpod_token") or "").strip()
    if not runpod_token:
        return JSONResponse({"error": "errorMissingRunpodToken", "i18n_key": "errorMissingRunpodToken"}, status_code=400)

    if await check_runpod_balance(runpod_token) is None:
        return JSONResponse(
            {"error": "errorRunpodKeyVerifyFailed", "i18n_key": "errorRunpodKeyVerifyFailed"}, status_code=400
        )

    endpoint_result = await check_runpod_endpoint(runpod_token)
    if not endpoint_result["success"]:
        # The key already passed the balance check, so this is a RunPod or configuration
        # problem rather than a bad key; 5xx tells the cookie migration to retry later.
        return JSONResponse(
            {
    ...
    user_email = get_user_email(request)
    try:
        await runpod_key_store.save(refresh_token, runpod_token, user_email)
    except GoogleAPIError as exc:
        logger.error("Failed to store RunPod key on Drive for %s: %s", user_email, exc)
        return JSONResponse(
...
async def delete_runpod_key(request: Request):
    """Remove the user's stored RunPod key from their Drive and from this session."""
    if in_local_mode:
        raise HTTPException(status_code=400, detail="RunPod keys are not available in local mode")
    ...
    user_email = get_user_email(request)
    try:
        await runpod_key_store.delete(refresh_token, user_email)
    except GoogleAPIError as exc:
        logger.error("Failed to remove RunPod key from Drive for %s: %s", user_email, exc)
        return JSONResponse(
...
async def handle_transcoding(job_id: str):
    ...
    try:
        ...
        if not queued:
            ...
            await db.incr_stat("transcoding_jobs", 1)
            await db.incr_stat("transcoding_total_gb", file_size_gb)
            await db.incr_stat("transcoding_total_duration_seconds", duration)
...
async def validate_upload_request_metadata(
    ...
    *,
    ...
    # Check RunPod endpoint if the user has their own key (never set in local mode)
...
async def transcribe_job(job_desc):
    ...
    try:
        ...
        await db.incr_stat(f"jobs_transcribed_{job_desc.job_type}", 1)
