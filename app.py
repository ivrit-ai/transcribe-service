    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                log_message_in_session(f"Max serverless concurrency: {max_concurrency}")
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                log_message_in_session(f"Current total worker usage across {len(endpoints)} endpoints: {total_workers}")
                
                # Log individual endpoint details for debugging
                for endpoint in endpoints:
                    log_message_in_session(f"Endpoint {endpoint['name']}: workersMax={endpoint.get('workersMax', 0)}")
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                for endpoint in endpoints:
                    log_message_in_session(f"Endpoint: {endpoint['name']}")
                    if re.match(pattern_regex, endpoint["name"]):
                        log_message_in_session(f"Found endpoint: {endpoint['id']} ({endpoint['name']}) with template {endpoint['templateId']}")
                        return {
                            "id": endpoint["id"], 
                            "name": endpoint["name"],
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.delete(
                ...
                log_message_in_session(f"Successfully deleted endpoint: {endpoint_id}")
                return True
...
async def create_runpod_endpoint(api_key: str, template_id: str) -> Optional[dict]:
    ...
    if max_concurrency is None or current_usage is None:
        ...
        log_message_in_session(f"Concurrency calculation: max={max_concurrency}, current={current_usage}, available={available_workers}, setting workersMax={workers_max}")
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                log_message_in_session(f"Created autogenerated endpoint: {endpoint_id} ({endpoint_name})")
                return {
                    "id": endpoint_id,
                    "name": endpoint_name,
    ...
    try:
        ...
        if endpoint_info:
            ...
            if current_template_id == required_template_id:
                ...
                log_message_in_session(f"Template ID mismatch: current={current_template_id}, required={required_template_id}")
                
                # Delete the existing endpoint
                delete_success = await delete_runpod_endpoint(runpod_token, endpoint_info["id"])
                if not delete_success:
                    log_message_in_session(f"Failed to delete endpoint {endpoint_info['id']}, but proceeding with creation")
